...
/* ============================================================
   Admin bell with a live badge and a short preview list.

   Staff notifications are a shared inbox (audience = 'staff'),
   not tied to one admin's user_id, so every admin sees the same
   unread count and the same feed. New rows arrive over Supabase
   Realtime, so anyone with the dashboard open sees them without
   refreshing.
   ============================================================ */
...
export function AdminNotificationBell({ to = "/admin/notifications" }: { to?: string }) {
  ...
  const { data: items = [] } = useQuery({
    queryKey: ["admin-notification-bell"],
    queryFn: fetchRecent,
  ...
  // Live updates. Any new staff notification refreshes the list and
  // pulses the bell so it is noticed without a page reload, no matter
  // which admin page is open.
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        ...
        () => {
          qc.invalidateQueries({ queryKey: ["admin-notification-bell"] });
